...
type t = { max_height : int; head : node }
...
let create_dummy_node_array max_height =
  let arr =
    Array.init (max_height + 1) (fun _ ->
        create_new_node Int.max_int max_height)
  in
  arr
...
let compare_and_set_mark_ref (atomic, old_node, old_mark, node, mark) =
  ...
  in
  current.node == old_node && current.marked = old_mark
  && ((current.node == node && current.marked = mark) || set_mark_ref ())
...
let find { head; max_height } key =
  let rec search (pred, curr, succ, mark, level) =
    if mark then
      ...
      let curr = (Atomic.get pred.next.(level)).node in
      let { node = succ; marked = mark } = Atomic.get curr.next.(level) in
...
let remove sl key =
  let preds = create_dummy_node_array sl.max_height in
  let succs = create_dummy_node_array sl.max_height in
  let found = find_in (key, preds, succs, sl) in
  if not found then false
    ...
    (Atomic.get nodeToRemove.next.(0)).node |> update_bottom_level
