module Atomicskiplist = Lockfree.Atomicskiplist3
...
let tests_two_domains =
  ...
  [
    ...
    Test.make ~count:10000 ~name:"parallel_add_remove" (pair small_nat small_nat) (fun (npush1, npush2) ->
      ...
      let rec check_none_present l = match l with
      | h::t -> if Atomicskiplist.find sl h then false else check_none_present t
      | [] ->  true
    in
